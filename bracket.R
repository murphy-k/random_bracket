bracket_generate <-
  function() {
    brkt <-
      read.csv("2019_initial_bracket.csv",
               as.is = TRUE,
               stringsAsFactors = FALSE)
    source("calc_probs.R")
    score_data <-
      read.csv("NCAA_mens_tournament_scores.csv", stringsAsFactors = FALSE)
    probs <- calc_prob_table(score_data)
    
    
    brkt$teamname_orig <- brkt$teamname
    brkt$teamname <-
      paste0("(", brkt$seed, ") ", brkt$teamname_orig)
    ...
    for (game in first_round_games) {
      ...
      pct_upset <-
        probs$percent_upset[probs$matchup == matchup] / 100
      if (identical(pct_upset, numeric(0)))
        pct_upset <- .5
    ...
    if (length(unique(seeds)) > 1) {
      ...
      upset <- rbinom(1, 1, pct_upset) == 1
      champ_winner <-
        ifelse(upset, teams[which(seeds == max(seeds))],
               teams[which(seeds == min(seeds))])
    } else{
      # same seed, just a coin flip now...
      champ_winner <-
        ifelse(rbinom(1, 1, .5) > .5, teams[1], teams[2])
    }
    ...
    text(109.8, 32.5, champ_winner, cex = 1.5)
    dev.off()
    print(champ_winner)
