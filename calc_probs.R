...
calc_prob_table <- function(df) {
  ...
  df$matchup <-
    paste0(apply(df[, c("team1_rank", "team2_rank")], 1, min),
           " : ",
           apply(df[, c("team1_rank", "team2_rank")], 1, max))
  
  df$which_won <-
    ifelse(df$team1_score > df$team2_score, "team1", "team2")
  df$which_lost <-
    ifelse(df$team1_score > df$team2_score, "team2", "team1")
  df$won_rank <-
    ifelse(df$which_won == "team1", df$team1_rank, df$team2_rank)
  df$lost_rank <-
    ifelse(df$which_won == "team1", df$team2_rank, df$team1_rank)
  df$upset <- df$won_rank > df$lost_rank
  
  tmp <- by(df$upset, df$matchup, sum)
  matchup_tab <- data.frame(
    matchup = names(tmp),
