...
get_bracket <- function(year) {
  url <-
    paste0(
      "http://www.espn.com/mens-college-basketball/tournament/bracket/_/id/",
      year,
      "22"
    )
  html <- read_html(url)
  ...
  df <-
    rbind(
      c(parse_score(html %>% html_node("#match61")), round = "4"),
  ...
  for (i in 1:4) {
    for (j in 1:30) {
      if (j %in% c(1:8, 16:23))
        round <- "64"
      if (j %in% c(9:12, 24:27))
        round <- "32"
      if (j %in% c(13, 14, 28, 29))
        round <- "16"
      if (j %in% c(15, 30))
        round <- "8"
